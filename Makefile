...
ifeq ($(DEVICE),stm8l152c6)
    DEFINES += -DSTM8L15X_MD
endif

INCLUDE_DIR = -I$(SRC_DIR) -I$(INC_DIR) -I$(BASE_HEADERS)

LDFLAGS = -mstm8 -lstm8
CFLAGS_0 = --opt-code-size --all-callee-saves --stack-auto --fverbose-asm --no-peep

CFLAGS = --Werror --std-sdcc99 -mstm8 --verbose  $(DEFINES)
...
$(PROGRAM).elf: $(OBJECTS)
	$(CC) $(LDFLAGS) --debug --out-fmt-elf  $(subst $(SRC_DIR),$(TMP_DIR), $^) -o $(OUT_DIR)/$@
